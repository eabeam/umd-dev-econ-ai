...
% Pop-out aside: a little conversation box in the bottom-right corner,
% for the parenthetical remarks that don't belong in the body.
% Usage: \aside{text}  or  \aside[5cm]{longer text}
\newcommand{\aside}[2][4.2cm]{%
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor=south east, xshift=-1.5cm, yshift=0.85cm, rotate=1.5,
        draw=UVMGold, fill=LightGold, rounded corners=4pt, line width=0.8pt,
        text=DeepCharcoal, font=\footnotesize\itshape, align=left,
        text width=#1, inner sep=7pt,
        drop shadow={shadow xshift=1.5pt, shadow yshift=-1.5pt, opacity=0.25}]
    (asidebox) at (current page.south east) {#2};
  \node[anchor=south west, text=UVMGold, font=\small]
    at ([xshift=2pt, yshift=-2pt]asidebox.north west) {\faCommentDots};
\end{tikzpicture}}
...
\begin{itemize}
  ...
  \item Agents inherit your standards. Messy repo in, messy code out.
\end{itemize}

\aside{November-you has no memory of August-you. None.}
...
\begin{tcolorbox}[accentbox]
\centering
The rhythm: \textbf{checkpoint $\to$ delegate $\to$ read the diff $\to$ keep or discard.}\\[3pt]
You are never stuck with what the agent did.
\end{tcolorbox}

\aside{Discarding isn't defeat --- it's cheaper than repair.}
